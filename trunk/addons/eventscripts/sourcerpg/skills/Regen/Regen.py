...
config.addInfo(skillName, """This skill allows a player to regenerate a certain amount of health every
so often until their maximum health is reached.""")

""" Assign all the server variables """
maxLevel        = config.cvar("srpg_regenMax",               5, "The maximum level of the regen skill")
creditStart     = config.cvar("srpg_regenCreditsStart",      5, "The starting amount of credits for this skill")
creditIncrement = config.cvar("srpg_regenCreditsIncrement", 10, "How much the credits increment after the first level")
regenDelay      = config.cvar("srpg_regenDelayTimer",      1.0, "The time (in seconds) between each time a user's regen power heals the player") 
...
class RegenObject(object):
    ...
    def heal(self, amount):
        ...
        to their maximum health and stop the regeneration
